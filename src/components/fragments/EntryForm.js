...
class EntryForm extends Component {
  ...
  validateField = (fieldName) => {
    ...
    if (fieldName === "ClientName") {
      ...
      values.PaymentMethods.forEach((paymentMethod, index) => {
        const hasAmountPaidError =
          !paymentMethod.AmountPaid || paymentMethod.AmountPaid <= 0;
  ...
  validateForm = () => {
    return new Promise((resolve) => {
      ...
      if (hasServicesRenderedError)
        errors.ServicesRendered = hasServicesRenderedError;

      const hasPaymentMethodsErrors = this.validateField("PaymentMethods");
      if (hasPaymentMethodsErrors.length > 0)
        errors.PaymentMethods = hasPaymentMethodsErrors;
    ...
    this.validateForm().then((hasErrors) => {
      if (!hasErrors) {
        if (this.props.mode === "edit") {
          ...
          axios
            .put(
              `${process.env.REACT_APP_SERVICE_URL}/entries/${this.props.entryData._id}`,
              {
          ...
          axios
            .post(
              process.env.REACT_APP_SERVICE_URL + "/entries/",
              {
                entry: this.state.values,
              },
              {
    ...
    if (!this.state.values.ClientName && this.props.entryData.ClientName) {
      ...
      this.setState({
        values: {
          ...
          ServicesRendered,
          PaymentMethods:
            (PaymentMethods.length > 0 && PaymentMethods) || paymentMethods,
        },
      });
    }
  };
  ...
  render() {
    const { DateAdded, ClientName, ServicesRendered, PaymentMethods } =
      this.state.values;
      ...
      <>
        <form onSubmit={this.handleSubmit} className="entryform">
          ...
          <div className="entryform-field">
            ...
            <div className="entryform-inputarea">
              <input
                ...
                value={ClientName}
                onChange={(e) =>
                  this.handleChange("ClientName", e.target.value)
                }
              />
            </div>
          ...
          <div className="entryform-field">
            ...
            />
            <Error show={errors.ServicesRendered}>
              Please select services rendered for this client.
            </Error>
          </div>
                ...
                <div className="entryform-repeating_row-field">
                  ...
                  <Error
                    show={
                      ...
                      errors.PaymentMethods[index].PaymentType
                    }
                  >
                ...
                <div className="entryform-repeating_row-field">
                  ...
                  <Error
                    show={
                      errors.PaymentMethods &&
                      errors.PaymentMethods[index] &&
                      errors.PaymentMethods[index].AmountPaid
                    }
                  >
